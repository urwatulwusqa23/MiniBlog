...
@Component({
    ...
    <div class="follow-page">
      ...
      @if (loading()) {
        ...
        <div class="empty-state">
          ...
          <p>Not following anyone yet.<br>Explore the feed and connect!</p>
        </div>
      } @else {
        <div class="user-list">
          @for (u of users(); track u.id) {
            <div class="user-card glass">
              ...
              <button class="btn btn-danger btn-sm" (click)="unfollow(u)">
  ...
  styles: [`
    ...
    .user-card {
      ...
      border-radius: var(--radius-xl); padding: 14px 18px; transition: var(--transition);
    ...
    .user-name { font-size: 15px; font-weight: 700; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; &:hover { color: var(--accent); } }
...
export class FollowingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userSvc.getFollowing().subscribe({
  ...
  unfollow(u: User): void {
    this.userSvc.unfollow(u.id).subscribe({
      next: () => { this.users.update(list => list.filter(x => x.id !== u.id)); this.toast.info('Unfollowed'); },
